...
def find_similar_scenario(query, index, model, scenarios, responses, threshold=0.5):
    ...
    if similarity_score < threshold:
        ...
        # Integrating with Replicate's LLM
        prompt = f"The user described the following emergency: {query}. Based on the similar scenario '{scenario}' from my db, and the suggested response is '{response}' from my db (there is a chance that my database wouldn't have a good response for the scenario), provide an enhanced response that the AI receptionist can give to the user. If you think that the {query} is very much unrelated to any medical emergency then just return the statement repeat the question. "
        output = replicate.run(
            ...
            input={
                ...
                "min_tokens": 0,
                "temperature": 0.6,
                "system_prompt": "You are helpful AI Receptionist.",
                "length_penalty": 1,
